...
class Parsecsv {
	...
	public function parse ($input = null, $offset = null, $limit = null, $conditions = null) {
		...
		if ( !empty($input) ) {
			...
			if ( is_readable($input) ) {
				$this->csvdata = $this->parse_file($input);
			} else {
				$this->file_data = &$input;
				$this->csvdata = $this->parse_string();
			}
			if ( $this->csvdata === false ) return false;
		}
		return true;
	}
	 ...
	 */
	public function save ($file = null, $csvdata = array(), $append = false, $fields = array()) {
		if ( empty($file) ) $file = &$this->file;
		$mode = ( $append ) ? 'at' : 'wt' ;
		$is_php = ( preg_match('/\.php$/i', $file) ) ? true : false ;
		return $this->_wfile($file, $this->unparse($csvdata, $fields, $append, $is_php), $mode);
	}
	 ...
	 */
	public function output ($filename = null, $csvdata = array(), $fields = array(), $delimiter = null) {
		if ( empty($filename) ) $filename = $this->output_filename;
		if ( $delimiter === null ) $delimiter = $this->output_delimiter;
		$csvdata = $this->unparse($csvdata, $fields, null, null, $delimiter);
		if ( $filename !== null ) {
	...
	public function auto ($file = null, $parse = true, $search_depth = null, $preferred = null, $enclosure = null) {
		...
		for ( $i=0; $i < $strlen; $i++ ) {
			$ch = $csvdata{$i};
			$nch = ( isset($csvdata{$i+1}) ) ? $csvdata{$i+1} : false ;
			$pch = ( isset($csvdata{$i-1}) ) ? $csvdata{$i-1} : false ;
		...
		// parse data
		if ( $parse ) $this->csvdata = $this->parse_string();
	 ...
	 */
	private function parse_file ($file = null) {
		if ( $file === null ) $file = $this->file;
	 ...
	 * @return  2D array with CSV data, or false on failure
	 */
	private function parse_string ($csvdata = null) {
		if ( empty($csvdata) ) {
			if ( $this->_check_data() ) {
				$csvdata = &$this->file_data;
			} else return false;
		...
		for ( $i=0; $i < $strlen; $i++ ) {
			...
			if ( $ch == $this->enclosure ) {
				if ( !$enclosed ) {
					...
					for ( $x=($i+1); isset($csvdata{$x}) && ltrim($csvdata{$x}, $white_spaces) == ''; $x++ ) {}
					if ( $csvdata{$x} == $this->delimiter ) {
						$enclosed = false;
						$i = $x;
					} else {
						if ( $this->error < 1 ) {
	 ...
	 */
	private function unparse ( $csvdata = array(), $fields = array(), $append = false , $is_php = false, $delimiter = null) {
		if ( !is_array($csvdata) || empty($csvdata) ) $csvdata = &$this->csvdata;
	...
	private function load_data ($input = null) {
		...
		if ( !empty($csvdata) || $csvdata = $this->_rfile($file) ) {
			if ( $this->file != $file ) $this->file = $file;
			if ( preg_match('/\.php$/i', $file) && preg_match('/<\?.*?\?>(.*)/ims', $csvdata, $strip) ) {
				$csvdata = ltrim($strip[1]);
			}
			if ( $this->convert_encoding ) $csvdata = iconv($this->input_encoding, $this->output_encoding, $csvdata);
			if ( substr($csvdata, -1) != "\n" ) $csvdata .= "\n";
			$this->file_data = &$csvdata;
			return true;
		}
		return false;
	}
	 ...
	 */
	private function _validate_row_conditions ($row = array(), $conditions = null) {
		if ( !empty($row) ) {
			if ( !empty($conditions) ) {
	 ...
	 * @return  true of false
	 */
	private function _validate_row_condition ($row, $condition) {
		$operators = array(
			'=', 'equals', 'is',
			'!=', 'is not',
			'<', 'is less than',
	 ...
	 * @return  true of false
	 */
	private function _validate_offset ($current_row) {
	 ...
	 */
	private function _enclose_value ($value = null) {
		if ( $value !== null && $value != '' ) {
	 ...
	 * @return  true or false
	 */
	private function _check_data ($file = null) {
		if ( empty($this->file_data) ) {
			if ( $file === null ) $file = $this->file;
	 ...
	 */
	private function _check_count ($char, $array, $depth, $preferred) {
	...
	private function _rfile ($file = null) {
		if ( is_readable($file) ) {
			...
			$csvdata = fread($fh, filesize($file));
			fclose($fh);
			return $csvdata;
		}
		return false;
